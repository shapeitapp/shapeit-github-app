const { findCommentsQuery, createCommentMutation, getIssue, itemProjectQuery } = require('./queries')
...
const handleNewScope = async(context, owner, repo, issueNodeId, issueNumber) => {
  ...
  const commentHeader = 'Thanks for creating a new scope'
...
\`\`\`
    `
  if (isScope(context.payload.issue.body)) {
    await addComment(context, owner, repo, issueNodeId, issueNumber, commentHeader, commentText)
